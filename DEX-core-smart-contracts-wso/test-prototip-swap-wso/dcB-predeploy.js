...
const fs = require('fs');
const pathJsonA = './DEXClientA.json';
...
async function main(client) {
  ...
  const clientAddrA = JSON.parse(fs.readFileSync(pathJsonA,{encoding: "utf8"})).address;


  const clientKeysB = signerKeys(await TonClient.default.crypto.generate_random_sign_keys());

  let status = false;
  let targetShard = getShard(clientAddrA);
  let n = 0;
  while (!status) {
    const clientAccB = new Account(DEXClientContract, {signer:clientKeysB,initData:{soUINT:n,codeDEXConnector:DEXConnectorContract.code},client,});
    const clientAddrB = await clientAccB.getAddress();
    let shard = getShard(clientAddrB);
    if (shard == targetShard) {
      console.log("Bingo!");
      console.log("clientSoArg:", n);
      console.log("clientAddress:", clientAddrB);
      let keyJson = JSON.stringify({address:clientAddrB, keys:clientKeysB, clientSoArg:n});
      fs.writeFileSync( pathJsonB, keyJson,{flag:'w'});
      console.log("Address of the DEXclientB and clientSoArg written successfully to:", pathJsonB);
      status = true;
    } else {console.log(n);}
    n++;
  }
