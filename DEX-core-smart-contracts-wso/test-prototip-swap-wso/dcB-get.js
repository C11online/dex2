...
const fs = require('fs');
const pathJson = './DEXClientB.json';
const path = './DEXConnectorB.json';
...
let n = response.decoded.output.counterCallback;
let idArr = [];
for (i=1;i<=n;i++){idArr.push(i)}
console.log("Contract reacted to your idArr:", idArr);
for (const item of idArr) {
  // Execute `counterCallback` get method  (execute the message locally on TVM)
  response = await clientAcc.runLocal("getCallback", {id:item});
