...
const name = toHex("DS-BTC/USDC");
const symbol = toHex("DS-BTC/USDC");
const decimals = 9;
...
async function main(client) {
  ...
  const rootA = JSON.parse(fs.readFileSync(pathJsonBTC,{encoding: "utf8"})).address;
  const rootA_Acc = new Account(RootTokenContract, {address: rootA,client,});
  const rootB = JSON.parse(fs.readFileSync(pathJsonUSDC,{encoding: "utf8"})).address;
  ...
  while (!status) {
    ...
    if (shardC == targetShard) {
      ...
      if (shardW == targetShard) {
        ...
        fs.writeFileSync( pathJsonPairBtcUsdc, keyJson,{flag:'w'});
        console.log("Data for the DEXpair written successfully to:", pathJsonPairBtcUsdc);
        status = true;
      } else {console.log(n);}
    } else {console.log(n);}
    n++;
