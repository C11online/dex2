...
async function main(client) {
  ...
  let usdprovide = 4000000;
  let usdt = usdprovide / 2;
  let usdc = usdprovide / 2;
  let ethprovide = 2 * ((usdprovide / 4) / ethusd);
  let btcprovide = 2 * ((usdprovide / 4) / btcusd);
  usdprovide = convert(usdprovide,9);
  usdtprovide = convert(usdt,9);
  usdcprovide = convert(usdc,9);
  ethprovide = convert(ethprovide,9);
  btcprovide = convert(btcprovide,9);
  console.log(usdtprovide,' nano wUSDT');
  console.log(usdcprovide,' nano wUSDC');
  ...
  response = await rootAccA.run("mint", {tokens:usdtprovide, to:walletRootA});
  console.log(`Contract rootAccA run mint with tx ${response.decoded.output}, ${response.transaction.id}`);

  response = await rootAccB.run("mint", {tokens:usdcprovide, to:walletRootB});
  console.log(`Contract rootAccB run mint with tx ${response.decoded.output}, ${response.transaction.id}`);

  response = await rootAccC.run("mint", {tokens:btcprovide, to:walletRootC});
  console.log(`Contract rootAccC run mint with tx ${response.decoded.output}, ${response.transaction.id}`);

  response = await rootAccD.run("mint", {tokens:ethprovide, to:walletRootD});
  console.log(`Contract rootAccD run mint with tx ${response.decoded.output}, ${response.transaction.id}`);
