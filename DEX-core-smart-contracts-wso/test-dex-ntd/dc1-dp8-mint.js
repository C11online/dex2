...
async function main(client) {
  ...
  // console.log(btcprovide,' nano wETH');

  let usdprovide = 2000000;
  usdprovide = convert(usdprovide,9);
  console.log(usdprovide,' nano wUSDC');
  console.log(usdprovide,' nano wUSDT');


  const rootKeysA = JSON.parse(fs.readFileSync(pathJsonUSDC,{encoding: "utf8"})).keys;
  const rootAddrA = JSON.parse(fs.readFileSync(pathJsonUSDC,{encoding: "utf8"})).address;
  const rootAccA = new Account(RootTokenContract, {address: rootAddrA,signer:rootKeysA,client,});
  console.log("WTON root:", rootAddrA);


  const rootKeysB = JSON.parse(fs.readFileSync(pathJsonUSDT,{encoding: "utf8"})).keys;
  const rootAddrB = JSON.parse(fs.readFileSync(pathJsonUSDT,{encoding: "utf8"})).address;
  const rootAccB = new Account(RootTokenContract, {address: rootAddrB,signer:rootKeysB,client,});
  console.log("BTC root:", rootAddrB);
  ...
  response = await rootAccA.run("mint", {tokens:usdprovide, to:walletRootA});
