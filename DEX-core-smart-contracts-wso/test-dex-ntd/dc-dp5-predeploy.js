...
const name = toHex("DS-ETH/USDC");
const symbol = toHex("DS-ETH/USDC");
const decimals = 9;
  ...
  while (!status) {
    ...
    if (shardC == targetShard) {
      ...
      if (shardW == targetShard) {
        ...
        fs.writeFileSync( pathJsonPairEthUsdc, keyJson,{flag:'w'});
        console.log("Data for the DEXpair written successfully to:", pathJsonPairEthUsdc);
        status = true;
      } else {console.log(n);}
    } else {console.log(n);}
    n++;
