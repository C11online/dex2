...
async function main(client) {
  ...
  const rootKeysB = JSON.parse(fs.readFileSync(pathJsonBTC,{encoding: "utf8"})).keys;
  const rootAddrB = JSON.parse(fs.readFileSync(pathJsonBTC,{encoding: "utf8"})).address;
  ...
  response = await rootAccB.run("mint", {tokens:btcprovide, to:walletRootB});
