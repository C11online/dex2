...
const name = toHex("DS-ETH/USDT");
const symbol = toHex("DS-ETH/USDT");
const decimals = 9;
...
async function main(client) {
  ...
  const rootA = JSON.parse(fs.readFileSync(pathJsonETH,{encoding: "utf8"})).address;
  ...
  while (!status) {
    ...
    if (shardC == targetShard) {
      ...
      if (shardW == targetShard) {
        ...
        fs.writeFileSync( pathJsonPairEthUsdt, keyJson,{flag:'w'});
        console.log("Data for the DEXpair written successfully to:", pathJsonPairEthUsdt);
        status = true;
      } else {console.log(n);}
    } else {console.log(n);}
    n++;
