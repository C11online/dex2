...
async function main(client) {
  ...
  const usdcAcc = new Account(RootTokenContract, {address: usdcAddr,client,});
  ...
  n = 0;
  while (!status) {
    response = await rootAcc.runLocal("getPairAddress", {_answer_id:0,pairPubKey:pubkey,pairSoArg:n,pairCreator:clientAddr,pairRootA:wtonAddr,pairRootB:usdcAddr,pairRootAB:rootABaddress});
  ...
  while (!status) {
    ...
    if (shardC == targetShard) {
      ...
      response = await wtonAcc.runLocal("getWalletAddress", {_answer_id:0, wallet_public_key_:0,owner_address_:connectorAddr});
  ...
  while (!status) {
    ...
    if (shardC == targetShard) {
      console.log("connectorSoArg:", n);
      console.log("getConnectorAddress:", connectorAddr);
      response = await usdcAcc.runLocal("getWalletAddress", {_answer_id:0, wallet_public_key_:0,owner_address_:connectorAddr});
      let walletAddr = response.decoded.output.value0;
      let shardW = getShard(walletAddr);
      if (shardW == targetShard) {
        console.log("Bingo!");
        ...
        fs.writeFileSync( pathJsonPairTonUsdc, keyJson,{flag:'w'});
        console.log("Data for the DEXpair written successfully to:", pathJsonPairTonUsdc);
        status = true;
      } else {console.log(n);}
    } else {console.log(n);}
    n++;
