...
async function main(client) {
  ...
  let ethprovide = usdprovide / ethusd;
  ...
  const rootKeysB = JSON.parse(fs.readFileSync(pathJsonETH,{encoding: "utf8"})).keys;
  const rootAddrB = JSON.parse(fs.readFileSync(pathJsonETH,{encoding: "utf8"})).address;
  ...
  response = await rootAccB.run("mint", {tokens:ethprovide, to:walletRootB});
