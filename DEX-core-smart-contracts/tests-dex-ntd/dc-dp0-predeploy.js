...
const name = toHex("DS-WTON/USDT");
const symbol = toHex("DS-WTON/USDT");
const decimals = 9;
...
async function main(client) {
  ...
  n = 0;
  while (!status) {
    response = await rootAcc.runLocal("getPairAddress", {_answer_id:0,pairPubKey:pubkey,pairSoArg:n,pairCreator:clientAddr,pairRootA:wtonAddr,pairRootB:usdtAddr,pairRootAB:rootABaddress});
  ...
  while (!status) {
    ...
    if (shardC == targetShard) {
      console.log("connectorSoArg:", n);
      console.log("getConnectorAddress:", connectorAddr);
      response = await usdtAcc.runLocal("getWalletAddress", {_answer_id:0, wallet_public_key_:0,owner_address_:connectorAddr});
      let walletAddr = response.decoded.output.value0;
      let shardW = getShard(walletAddr);
      if (shardW == targetShard) {
        console.log("Bingo!");
        ...
        fs.writeFileSync( pathJsonPairTonUsdt, keyJson,{flag:'w'});
        console.log("Data for the DEXpair written successfully to:", pathJsonPairTonUsdt);
        status = true;
      } else {console.log(n);}
    } else {console.log(n);}
    n++;
