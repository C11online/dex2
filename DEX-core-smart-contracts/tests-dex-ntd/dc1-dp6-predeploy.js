...
async function main(client) {
  ...
  const rootB = JSON.parse(fs.readFileSync(pathJsonETH,{encoding: "utf8"})).address;
  ...
  while (!status) {
    ...
    if (shardC == targetShard) {
      ...
      if (shardW == targetShard) {
        ...
        fs.writeFileSync( pathJsonPairTonEth, keyJson,{flag:'w'});
        console.log("Data for the DEXpair written successfully to:", pathJsonPairTonEth);
        status = true;
      } else {console.log(n);}
    } else {console.log(n);}
    n++;
